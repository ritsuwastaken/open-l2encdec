...
#include <mbedtls/bignum.h>
    ...
    constexpr bool RSA_ENABLE_THREADS = false;
#elif defined(__cpp_lib_thread)
    constexpr bool RSA_ENABLE_THREADS = true;
#else
    constexpr bool RSA_ENABLE_THREADS = false;
#endif

#if RSA_ENABLE_THREADS
#include <atomic>
#include <thread>
#endif
...
size_t rsa::add_padding(std::vector<uint8_t> &output,
                        const std::vector<uint8_t> &input)
{
    size_t input_offset = 0;
    size_t num_blocks =
        (input.size() + BLOCK_BODY_SIZE - 1) / BLOCK_BODY_SIZE;

    output.resize(num_blocks * BLOCK_SIZE);

    for (size_t output_offset = 0;
         input_offset < input.size();
         output_offset += BLOCK_SIZE)
    {
        size_t chunk_size =
            std::min(input.size() - input_offset, BLOCK_BODY_SIZE);
        ...
        size_t data_offset =
            output_offset + BLOCK_SIZE - align_to_4_bytes(chunk_size);

        std::copy(input.begin() + input_offset,
                  input.begin() + input_offset + chunk_size,
...
size_t rsa::remove_padding(std::vector<uint8_t> &output,
                           const std::vector<uint8_t> &input)
{
    output.clear();

    for (size_t offset = 0;
         offset + BLOCK_SIZE <= input.size();
         offset += BLOCK_SIZE)
    {
        ...
        size_t data_offset =
            offset + BLOCK_SIZE - align_to_4_bytes(chunk_size);

        if (data_offset + chunk_size > input.size())
            break;

        output.insert(output.end(),
                      input.begin() + data_offset,
                 ...
                 const std::string &public_exp_hex)
{
    ...
    size_t total_size = add_padding(padded, input_data);
    if (total_size == 0)
        return -1;
    ...
    size_t num_threads =
        std::min({hw ? hw : NUM_THREADS, NUM_THREADS, total_blocks});
    if (num_threads == 0)
        num_threads = 1;
    ...
    {
        if (mbedtls_mpi_copy(&thread_mods[i].v, &modulus.v) != 0 ||
            mbedtls_mpi_copy(&thread_exps[i].v, &public_exp.v) != 0)
            return -3;
    }
    ...
    for (size_t t = 0; t < num_threads; ++t)
    {
        threads.emplace_back([&, t]()
            ...
            while (true)
            {
                ...
                if (i >= total_blocks || error.load() != 0)
                    break;

                size_t offset = i * BLOCK_SIZE;

                int rc = mbedtls_mpi_read_binary(
                    &block.v, padded.data() + offset, BLOCK_SIZE);
                if (rc != 0) { store_first_error(error, rc); break; }

                rc = mbedtls_mpi_exp_mod(
                    &encrypted.v, &block.v,
                    &thread_exps[t].v,
                    &thread_mods[t].v, nullptr);
                if (rc != 0) { store_first_error(error, rc); break; }

                rc = mbedtls_mpi_write_binary(
                    &encrypted.v, temp.data(), BLOCK_SIZE);
                if (rc != 0) { store_first_error(error, rc); break; }
    ...
    for (size_t i = 0; i < total_blocks; ++i)
    {
        ...
        rc = mbedtls_mpi_exp_mod(
            &encrypted.v, &block.v,
            &public_exp.v, &modulus.v, nullptr);
        if (rc != 0) return rc;
                 ...
                 const std::string &private_exp_hex)
{
    ...
    std::vector<unsigned char> decrypted(input_data.size());
    ...
    {
        if (mbedtls_mpi_copy(&thread_mods[i].v, &modulus.v) != 0 ||
            mbedtls_mpi_copy(&thread_privs[i].v, &private_exp.v) != 0)
            return -3;
    }
    ...
    for (size_t t = 0; t < num_threads; ++t)
    {
        threads.emplace_back([&, t]()
            ...
            while (true)
            {
                ...
                int rc = mbedtls_mpi_read_binary(
                    &block.v, input_data.data() + offset, BLOCK_SIZE);
                if (rc != 0) { store_first_error(error, rc); break; }

                rc = mbedtls_mpi_exp_mod(
                    &decrypted_block.v, &block.v,
                    &thread_privs[t].v,
                    &thread_mods[t].v, nullptr);
                if (rc != 0) { store_first_error(error, rc); break; }

                rc = mbedtls_mpi_write_binary(
                    &decrypted_block.v, temp.data(), BLOCK_SIZE);
                if (rc != 0) { store_first_error(error, rc); break; }

                std::copy(temp.begin(), temp.end(),
                          decrypted.begin() + offset);
            } });
    }
    ...
    for (size_t i = 0; i < total_blocks; ++i)
    {
        ...
        rc = mbedtls_mpi_exp_mod(
            &decrypted_block.v, &block.v,
            &private_exp.v, &modulus.v, nullptr);
        if (rc != 0) return rc;
    ...
    output_data.clear();
    remove_padding(output_data, decrypted);
    return 0;
}

...
static bool FindExtension(const char *Ext, const char **ExtensionsList, int NumExtensions)
{
	for (int i = 0; i < NumExtensions; i++)
		if (!stricmp(Ext, ExtensionsList[i]))
			return true;
	return false;
}
	...
	while (true)
	{
		...
		if (MayHaveExtension)
		{
			...
			if (c == 0)
			{
				// End of string
				if (len < 0)
					len = s - FileName; // preserve strrchr('.') logic
				break;
			}
		}
		else
...
CGameFileInfo *CGameFileInfo::Register(FVirtualFileSystem *parentVfs, const CRegisterFileInfo &RegisterInfo)
{
	...
	if (!parentVfs && !IsPackage)
	{
...
#endif
		{
			...
			if (appStristr(RegisterInfo.Filename, "cooked") || appStristr(RegisterInfo.Filename, "content"))
				return NULL;
	...
	if (info->Size < 16)
		info->Flags &= ~CGameFileInfo::GFI_Package;
...
void appSetRootDirectory(const char *dir, bool recurse)
{
	...
	if (dir[0] == 0)
		dir = "."; // using dir="" will cause scanning of "/dir1", "/dir2" etc (i.e. drive root)
...
const char *appSkipRootDir(const char *Filename)
{
	...
	while (true)
	{
		...
		if (!c2)
			return Filename; // Filename is shorter than GRootDirectory
		if (c1 != c2)
			return Filename; // different names
	}
}
...
void appEnumGameFilesWorker(EnumGameFilesCallback_t Callback, const char *Ext, void *Param)
{
	...
	for (const CGameFileInfo *info : GameFiles)
	{
		...
		{
			// check extension
			if (stricmp(info->GetExtension(), Ext) != 0)
				continue;
		}
		if (!Callback(info, Param))
			break;
	}
	unguard;
}
...
void appEnumGameFoldersWorker(EnumGameFoldersCallback_t Callback, void *Param)
{
	...
	{
		if (!Callback(info.Name, info.NumFiles, Param))
			break;
	}
	unguard;
}

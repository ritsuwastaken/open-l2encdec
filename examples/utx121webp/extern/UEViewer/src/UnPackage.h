...
#if UNREAL4
		if (Ar.Game >= GAME_UE4_BASE && Ar.ArVer >= 196 /*VER_UE4_REMOVE_NET_INDEX*/)
			return Ar;
#endif
#if UNREAL3
...
protected:
	...
	UnPackage() : FilenameNoInfo(NULL), Name(NULL), FileInfo(NULL), Loader(NULL), NameTable(NULL), ImportTable(NULL), ExportTable(NULL)
#if UNREAL4
				  ,
...
public:
	...
	static UnPackage *LoadPackageFromMemory(const char *Name, const std::vector<unsigned char> &Data, int Game);
...
public:
	...
	virtual bool ContainsEditorData() const
	{
		if (Game < GAME_UE4_BASE)
			return false;
		if (Summary.IsUnversioned)
			return false; // unversioned packages definitely has no editor data

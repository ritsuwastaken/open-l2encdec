...
#define EDITOR_STR ""
			...
			unguardf("%s'%s.%s', pos=%X, ver=%d/%d%s%s", Obj->GetClassName(), Package->Name, Obj->Name, Package->Tell(),
					 Package->ArVer, Package->ArLicenseeVer, UNVERS_STR, EDITOR_STR);
		}
		// postload objects
...
struct FPropertyTag
{
	...
	friend FArchive &operator<<(FArchive &Ar, FPropertyTag &Tag)
	{
		...
		case 4:
			...
			break;
		case 5:
			Ar << *((byte *)&Tag.DataSize);
			break;
		case 6:
			Ar << *((uint16 *)&Tag.DataSize);
			break;
		case 7:
			Ar << *((int32 *)&Tag.DataSize);
			break;
		}
...
struct FPropertyTagBat2
{
	...
	friend FArchive &operator<<(FArchive &Ar, FPropertyTagBat2 &Tag)
	{
		...
		if (!Tag.Type)
			return Ar;
		if (Ar.Game == GAME_Batman3 && Ar.ArLicenseeVer >= 104)
			goto named_prop; // Batman3 Online has old version of FPropertyTag
		Ar << Tag.Offset;
		...
		if (Ar.Game == GAME_Batman4 && Tag.Type == NAME_BoolProperty)
			goto simple_prop; // serialized as 4 byte int, i.e. a bunch of boolean properties
...
#undef END
	...
	while (p < end)
	{
		...
		if (IsOurClass)
			break; // the class has been verified, and we didn't find a property
		p++;	   // skip END marker
	}
...
#define SIMPLE_ARRAY_TYPE(TypeConst, CppType)    \
	if (COMPARE_TYPE(Prop->TypeName, TypeConst)) \
	{                                            \
		Ar << *(TArray<CppType> *)Arr;           \
	}

		SIMPLE_ARRAY_TYPE(PropType::Int, int)
		else SIMPLE_ARRAY_TYPE(PropType::Bool, bool) else SIMPLE_ARRAY_TYPE(PropType::Byte, byte) else SIMPLE_ARRAY_TYPE(PropType::Float, float) else SIMPLE_ARRAY_TYPE(PropType::UObject, UObject *) else SIMPLE_ARRAY_TYPE(PropType::FName, FName) else SIMPLE_ARRAY_TYPE(PropType::FVector, FVector) else SIMPLE_ARRAY_TYPE("FQuat", FQuat) else
		{
	...
	case NAME_EnumProperty:
	{
		FName EnumValue;
		Ar << EnumValue;
		assert(Prop->TypeName[0] == '#') int tmpInt = NameToEnum(Prop->TypeName + 1, *EnumValue);
		if (tmpInt == ENUM_UNKNOWN)
			appNotify("unknown member %s of enum %s", *EnumValue, Prop->TypeName + 1);
		assert(tmpInt >= 0 && tmpInt <= 255);
		*value = tmpInt;
...
#define PROP_DBG(fmt, ...)
	...
	while (Header.GetNextProperty(PropIndex, bIsZeroedProp))
	{
		...
		if (Prop->Count == -1)
		{
			...
			SIMPLE_ARRAY_TYPE(PropType::Int, int)
			else SIMPLE_ARRAY_TYPE(PropType::Bool, bool) else SIMPLE_ARRAY_TYPE(PropType::Byte, byte) else SIMPLE_ARRAY_TYPE(PropType::Float, float) else SIMPLE_ARRAY_TYPE(PropType::UObject, UObject *) else SIMPLE_ARRAY_TYPE(PropType::FName, FName) else SIMPLE_ARRAY_TYPE(PropType::FVector, FVector) else SIMPLE_ARRAY_TYPE("FQuat", FQuat) else bSerialized = false;
			if (bSerialized)
			{
#if DEBUG_PROPS
				appPrintf("  %s[%d] {}\n", PropName, Arr->Num());

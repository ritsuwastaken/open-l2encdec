...
#endif
#if A51
	if (Ar.Game == GAME_A51 && Ar.ArVer >= 376)
		goto new_ver; // partially upgraded old engine
#endif				  // A51
#if STRANGLE
	if (Ar.Game == GAME_Strangle)
		goto new_ver; // also check package's MidwayTag ("WOO ") and MidwayVer (>= 369)
#endif
#if AVA
	if (Ar.Game == GAME_AVA && Ar.ArVer >= 436)
		goto new_ver;
#endif
#if DOH
	if (Ar.Game == GAME_DOH)
...
FArchive &operator<<(FArchive &Ar, FString &S)
{
	...
	if (Ar.Game >= GAME_UE3)
		goto ue3; // just a shortcut for UE3 and UE4
...
FFileArchive::FFileArchive(const char *Filename, EFileArchiveOptions InOptions)
	: Options(InOptions), f(NULL), Buffer(NULL), BufferSize(0), BufferPos(0), FilePos(0)
{
...
int FFileArchive::GetFileSize() const
{
	...
	if (size >= MAX_FILE_SIZE_32)
		appError("GetFileSize returns 0x%llX", size); // 2Gb size restriction
	return (int)size;
}
...
FFileReader::FFileReader(const char *Filename, EFileArchiveOptions InOptions)
	: FFileArchive(Filename, InOptions), SeekPos(-1), FileSize(-1), BufferBytesLeft(0), LocalReadPos(0)
{
...
FFileWriter::FFileWriter(const char *Filename, EFileArchiveOptions InOptions)
	: FFileArchive(Filename, InOptions), FileSize(0), ArPos64(0)
{
...
#endif
#if HAWKEN
	else if (Ar.Game == GAME_Hawken && H.Tag == 0xEA31928C)
		goto tag_ok;
#endif
#if MMH7
	else if (/*Ar.Game == GAME_MMH7 && */ H.Tag == 0x4D4D4837)
		goto tag_ok; // Might & Magic Heroes 7
#endif
#if SPECIAL_TAGS
	else if (H.Tag == 0x7E4A8BCA)
		goto tag_ok; // iStorm
#endif
	else
...
void FByteBulkData::SerializeDataChunk(FArchive &Ar)
{
	...
	if (BulkDataFlags & (BULKDATA_CompressedLzo | BULKDATA_CompressedZlib | BULKDATA_CompressedLzx))
	{
		// compressed block
		int flags = 0;
		if (BulkDataFlags & BULKDATA_CompressedZlib)
			flags = COMPRESS_ZLIB;
		if (BulkDataFlags & BULKDATA_CompressedLzo)
			flags = COMPRESS_LZO;
		if (BulkDataFlags & BULKDATA_CompressedLzx)
			flags = COMPRESS_LZX;
